...
module.exports = function watsonSpeechToText(options={}) {
  ...
  let stream, socket, state = 'stopped' // state === stopped | starting | started | stopping
  ...
  const write = function(data) {
    // forward audio data to the watson stt backend while not in a stopped state.
    ...
    if (data.byteLength > 0 && (state === 'started' || state === 'starting'))
      _send(data)
  }
    ...
    return new Promise(function(resolve, reject) {
      ...
      socket.onmessage = function(data) {
        try {
          ...
          if (message.error) {
            publish('error', new Error(message.error))
