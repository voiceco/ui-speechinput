...
`
  ...
  const recording = function(stream) {
    ...
    canvas.width = 120 * ratio // dom.clientWidth
    canvas.height = 26 * ratio // dom.parentNode.clientHeight / 2
  ...
  const _draw = function() {
    ...
    const ratio = window.devicePixelRatio || 1
    let barWidth = canvas.width * 1.9 / bandCount
    //let fade = true

    for (let i = 0; i < bandCount; i++) {
      //let brightness = fade ? clamp(Math.floor(spectrum[i] / 1.5), 25, 99) : 99
      let barHeight = canvas.height * (Math.max(2, spectrum[i]) / 255)
